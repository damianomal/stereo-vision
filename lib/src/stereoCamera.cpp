...
//        std::cout << disp.size() << std::endl;
    ...
    if(debug_count == 100)
    {
        ...
        std::cout << std::endl << "---- STEREOCAM: AVERAGE TIMING OVER " << debug_count << " FRAMES ----" << std::endl;

        for(int i = 0; i < 4; i++)
        {
            std::cout << debug_strings_stereo[i] << ": " << (this->debug_timings[i] / debug_count) / 1000. << " ms" << std::endl;
